...
void control(Sai2Model::Sai2Model* robot, Simulation::Sai2Simulation* sim) {
	...
	// Eigen::Vector3d constraint_acc_in_task for extra credit;
	...
	while (fSimulationRunning) { //automatically set to false when simulation is quit
		...
		ee_vel_des <<0, 0.4 * (M_PI/3) * cos(2*M_PI*curr_time/6), 0;
		ee_acc_des <<0, -0.4 * (M_PI/3) * (M_PI/3)  * sin(2*M_PI*curr_time/6), 0;

		/* --------------------------------------------------------------------------------------
			FILL ME IN: compute joint torques
		-------------------------------------------------------------------------------------*/
		// constraint controller design
        n_c = (sphere_center - closest_point)/(sphere_center - closest_point).norm();
		Jc = n_c.transpose() * Jv_closest_point;

		// use equation on slide 41 of lecture 14 for potential field and take gradient of d to get constraint force
		constraint_force = -1 * eta * (1/closest_distance - 1/constraint_dist_thresh) * pow(1/closest_distance,2) ;

		//page 85 for kinetic energy matrix 
		Lambda_c =(Jc * robot->_M_inv * Jc.transpose()).inverse();;

		// use equation 5.6 jcbar, the dynamically consistent inverse of jc
		Jcbar = (robot->_M).inverse() * Jc.transpose() * Lambda_c;

		if(closest_distance < constraint_dist_thresh) {
			tau_c = Jc.transpose()*constraint_force + Jc.transpose() * Lambda_c * (-kvc * Jc * robot->_dq);
			Nc = (MatrixXd::Identity(robot->dof(), robot->dof()) - Jcbar * Jc);
			
		}
        else {tau_c = VectorXd::Zero(robot->dof());
			  Nc = MatrixXd::Identity(robot->dof(),robot->dof());
		}
		...
		MatrixXd F_s = ee_acc_des - kpx * (ee_pos - ee_pos_des) -kvx * (ee_vel - ee_vel_des);

		robot->Jv(Jv, ee_link_name, ee_pos_local);
		MatrixXd J_tc = Jv * Nc;
		
		// find constraint consistent task space inertia lambda_tc
		MatrixXd lambda_tc = (J_tc * robot->_M.inverse() * J_tc.transpose()).inverse();
		

		
		MatrixXd tau_tc = J_tc.transpose() * (lambda_tc * (F_s /*J_tc * robot->_M.inverse() * tau_c )*/));


		MatrixXd J_tc_bar = robot->_M.inverse() * J_tc.transpose() * lambda_tc;

		MatrixXd Nt = (MatrixXd::Identity(robot->dof(), robot->dof()) - J_tc_bar * J_tc);

		//equation1 in lecture 13 slide 28
		MatrixXd tau_ptc = Nc.transpose() * Nt.transpose() * robot->_M * (-kpj*(robot->_q - q_des) - kvj*robot->_dq);
